...
exp_data = read.table("./source_data/alcala_mapped.csv", header = T, sep = ",", row.names = 1, check.names = F)
clinical = read.table("./Carcinoid_Alcala_pan_sample/Sample_annotation.tsv", header = T, sep = "\t", row.names = 1, check.names = F)
clinical2= as.data.frame(readxl::read_xlsx("./Carcinoid_Alcala_pan_sample/Carcinoid_LCNEC_meta.xlsx"))

clinical2 = clinical2[2:nrow(clinical2),]
rownames(clinical2) = clinical2[,1]
clinical2[,1] = NULL

clinical2 = clinical2[1:(nrow(clinical2)-1),]
# Step 1: Initialize vectors with NA of the correct length
clinical$Survival_censor <- rep(NA, nrow(clinical))
clinical$Survival_months <- rep(NA, nrow(clinical))

# Step 2: For matching rows, copy values from clinical2 to clinical
matching_rows <- match(rownames(clinical2), rownames(clinical))
# Note: 'matching_rows' will contain NA for non-matching elements, which we need to exclude

# Only proceed with indexes that are not NA (i.e., there is a match)
valid_indexes <- !is.na(matching_rows)

# Update Survival_censor and Survival_months for matching row names
clinical$Survival_censor[matching_rows[valid_indexes]] <- clinical2$Survival_censor[valid_indexes]
clinical$Survival_months[matching_rows[valid_indexes]] <- clinical2$Survival_months[valid_indexes]
...
write.csv(clinical, file = "./source_data/alcala_clinical_anno.csv", row.names = TRUE)
