...
setwd("/home/hcemm-user/SurvSig/SurvSig/source_data/")


exp_data = read.table("./george_mapped.csv", header = T, sep = ",", row.names = 1, check.names = F)
clinical = data.frame(row.names = colnames(exp_data))

#################### EMT Score ####################

load(file='./EMT_gene_list.rda')
gene_list = EMT_gene_list
write.csv(gene_list, file = "./emt_gene_list.csv", row.names = TRUE)
...
gene_list <- read.table("NE50-signatures.txt", header = TRUE, sep = "\t", comment.char = "")
gene_names <- gene_list$Symbol

ne50signature_file <- "NE50-signatures.txt"
...
write.csv(clinical, file = "./george_sclc_anno.csv", row.names = TRUE)
