setwd("/home/kolos/website_test_ccapp_v2_lite/")

exp_data = read.table("./source_data/rousseaux_mapped.csv", header = T, sep = ",", row.names = 1, check.names = F)
exp_data = na.omit(exp_data)
clinical = read.table("./source_data/GSE30219.clinical_table.tsv", header = T, sep = "\t", row.names = 1, check.names = F)
...
gene_list[gene_list$genes == "GPR56", "Symbol"] = "ADGRG1"
...
gene_list[gene_list$Symbol == "CYR61", "Symbol"] = "CCN1"
gene_list[gene_list$Symbol == "FAM57B", "Symbol"] = "TLCD3B"
...
write.csv(clinical, file = "./source_data/rousseaux_clinical_anno.csv", row.names = TRUE)
write.csv(exp_data, file = "./source_data/rousseaux_mapped.csv", row.names = TRUE)
