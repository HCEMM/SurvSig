...
exp_data = read.table("./source_data/liu_mapped.csv", header = T, sep = ",", row.names = 1, check.names = F)
clinical = read.table("./source_data/liu_anno_clinical.csv", header = T, sep = ",", row.names = 1, check.names = F)
rownames(clinical) <- paste0("T", rownames(clinical))
clinical = clinical[order(rownames(clinical)),]
colnames(exp_data)[!colnames(exp_data) %in% rownames(clinical)]

clinical$Gender = ifelse(as.character(clinical$Gender) == 'F', "Female", "Male")

clinical[,c(2, 3, 4, 5, 8, 10, 11, 12, 13, 14, 16, 17, 18, 19)] = NULL
...
write.csv(clinical, file = "./source_data/liu_clinical_anno.csv", row.names = TRUE)
